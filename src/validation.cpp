...
bool CheckTransaction(const CTransaction& tx, CValidationState &state, bool fCheckDuplicateInputs)
{
    ...
    if (txsize > MAX_TX_BASE_SIZE)
        return state.DoS(100, false, REJECT_INVALID, "bad-txns-oversize");
    if (txsize < MIN_TX_BASE_SIZE)
    ...
    if (consensus.EnableNativeTokenHeight < 0 || chainActive.Height() <= consensus.EnableNativeTokenHeight)
...
bool IsASERTEnabled(const Consensus::Params& params, int32_t nHeight) {
    return nHeight >= params.asertAnchorParams.nHeight;
}
                              ...
                              bool fOverrideMempoolLimit, const CAmount& nAbsurdFee, std::vector<uint256>& vHashTxnToUncache)
{
...

    {
        ...
        if (nSigOpsCount > MAX_STANDARD_TX_SIGOPS_COUNT)
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    // Subsidy is cut in half every 210,000 blocks which will occur approximately every 1 years.
    nSubsidy >>= halvings;
    return nSubsidy;
}
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    // Also, currently the rule against blocks more than 2.5 minutes in the future
    ...
    unsigned int flags = SCRIPT_VERIFY_SIGPUSHONLY | SCRIPT_VERIFY_STRICTENC | SCRIPT_VERIFY_LOW_S | SCRIPT_VERIFY_NULLFAIL;
    ...
    for (unsigned int i = 0; i < block.vtx.size(); i++)
    {
        ...
        // GetTransactionSigOpCount counts 1 types of sigops:
        // * legacy (always)
        nSigOpsCount += GetTransactionSigOpCount(tx);
        if (nSigOpsCount > MAX_BLOCK_SIGOPS_COUNT)
...
static bool ActivateBestChainStep(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexMostWork, const std::shared_ptr<const CBlock>& pblock, bool& fInvalidFound, ConnectTrace& connectTrace)
{
    ...
    if (fBlocksDisconnected) {
        mempool.removeForReorg(pcoinsTip, chainActive.Height() + 1, 0);
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if (nSigOps > MAX_BLOCK_SIGOPS_COUNT)
...
bool ContextualCheckBlockHeader(const CBlockHeader& block, CValidationState& state, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
    ...
    // Check timestamp
    if (block.GetBlockTime() > nAdjustedTime + 2 * 60 + 30)
