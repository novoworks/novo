 ...
 * between novod, and novo-qt and non-server tools.
*/
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), NOVO_CONF_FILENAME));
    if (mode == HMM_NOVOD)
    {
#if HAVE_DECL_DAEMON
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), NOVO_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/novoworks/novo>";
    const std::string URL_WEBSITE = "<https://novonode.org>";
...
/** Sanity checks
 *  Ensure that Novo is running in a usable environment with all
...
bool AppInitParameterInteraction()
{
    ...
    int64_t nMempoolSizeMin = GetArg("-limitdescendantsize", DEFAULT_DESCENDANT_SIZE_LIMIT) * 1000;
...
static bool LockDataDirectory(bool probeOnly)
{
    ...
    // Make sure only a single Novo process is using the data directory.
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(GetArg("-conf", NOVO_CONF_FILENAME)).string());
