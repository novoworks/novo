    ...
    ui(new Ui::HelpMessageDialog)
{
    ...
    if (about)
    {
        ...
        QString header = tr("Usage:") + "\n" +
            "  novo-qt [" + tr("command-line options") + "]                     " + "\n";
        ...
        std::string strUsage = HelpMessage(HMM_NOVO_QT);
        ...
        if (showDebug) {
            strUsage += HelpMessageOpt("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", NovoGUI::DEFAULT_UIPLATFORM));
        }
...
void PaperWalletDialog::on_getNewAddress_clicked()
{
    ...
    CNovoAddress pubkeyhash;
    pubkeyhash.Set(pubkey.GetID());

    // Create String versions of each
    std::string myPrivKey = CNovoSecret(privKey).ToString();
...
void PaperWalletDialog::on_printButton_clicked()
{
    ...
    while (true) {
        ...
        // Ask for an amount to send to each paper wallet. It might be better to try to use the NovoAmountField, but this works fine.
    ...
    if (txFee > 0) {
        ...
        questionString.append(NovoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), txFee));
    ...
    Q_FOREACH (NovoUnits::Unit u, NovoUnits::availableUnits()) {
        if (u != model->getOptionsModel()->getDisplayUnit())
            alternativeUnits.append(NovoUnits::formatWithUnit(u, totalAmount));
    }

    questionString.append(tr("Total Amount %1 (= %2)")
                              .arg(NovoUnits::formatWithUnit(model->getOptionsModel()->getDisplayUnit(), totalAmount))
...
QWidget *ShutdownWindow::showShutdownWindow(NovoGUI *window)
{
    if (!window)
        return nullptr;
