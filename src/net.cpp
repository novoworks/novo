...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Novo users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Novo " + FormatFullVersion();
...
bool CConnman::OutboundTargetReached(bool historicalBlockServingLimit)
{
    ...
    if (historicalBlockServingLimit)
    {
        ...
        uint64_t buffer = timeLeftInCycle / 600 * MAX_BLOCK_BASE_SIZE;
